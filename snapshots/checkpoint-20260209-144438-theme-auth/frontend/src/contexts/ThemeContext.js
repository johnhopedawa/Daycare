...
const applyTheme = (theme) => {
  ...
  const parentBorderBaseColor = palette.parent_border_base || palette.parent_card_border || palette.accent || palette.primary;
  const parentBorderRgb = hexToRgb(parentBorderBaseColor) || '243, 244, 246';

  const parentSoftBaseColor = palette.parent_soft_base || palette.primary || palette.accent;
  const parentSoftRgb = hexToRgb(parentSoftBaseColor) || '91, 188, 190';

  const parentPillBaseColor = palette.parent_pill_base || palette.primary;
  ...
  const parentInputBorder = palette.parent_input_border
    || '#e5e7eb';
  const parentButtonBg = palette.parent_button_bg || palette.primary || defaultTheme.palette.primary;
  const parentButtonHover = palette.parent_button_hover || palette.primary_dark || defaultTheme.palette.primary_dark;
  const parentButtonText = palette.parent_button_text || palette.on_primary || defaultTheme.palette.on_primary;
  const parentHeaderBg = palette.parent_header_bg || parentCardBgStrong;
  const parentHeaderBorder = palette.parent_header_border || parentCardBorder;
  const parentNavText = palette.parent_nav_text || parentTextMuted;
  const parentNavActiveText = palette.parent_nav_active_text || palette.primary_dark || palette.primary || parentText;
  const parentNavIndicator = palette.parent_nav_indicator || palette.primary || defaultTheme.palette.primary;
  const parentIconBg = palette.parent_icon_bg || parentSoftBg;
  const parentIconText = palette.parent_icon_text || parentText;
  ...
  const headingFont = sanitizeFontFamily(fonts.heading, defaultTheme.fonts.heading);
  ...
  const fontImportUrl = resolveThemeFontImportUrl({ ...fonts, headingFont, bodyFont, scriptFont });
  ...
  ensureFontImport(fontImportUrl);
};

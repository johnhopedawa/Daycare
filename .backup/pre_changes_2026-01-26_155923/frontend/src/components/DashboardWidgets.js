...
export function MetricCard({
  ...
  if (variant === 'neutral') {
    return (
      <motion.div
        ...
        className="border bg-[var(--surface)] shadow-sm"
        style={{
  ...
  return (
    <motion.div
      ...
      animate={{ y: 0, opacity: 1 }}
      transition={{ delay, duration: 0.5 }}
      className={`p-6 rounded-3xl ${cardClass} relative overflow-hidden group transition-all duration-300 cursor-default border themed-border shadow-[0_14px_30px_-22px_rgba(15,23,42,0.45)] hover:shadow-[0_18px_36px_-24px_rgba(15,23,42,0.5)]`}
      style={{
        ...cardStyle,
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-6">
        <h3 className="section-title font-quicksand font-bold text-lg">
          Today's Attendance
        </h3>
...
export function TimelineWidget() {
  ...
  return (
    ...
    >
      ...
      <div className="relative pl-4 border-l-2 themed-border space-y-6">
        {events.map((event, i) => (
    ...
    <div className="grid grid-cols-2 gap-3">
      {actions.map((action, i) => {
        ...
        <motion.button
          ...
          className="p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-shadow hover:shadow-md border themed-border"
          style={buttonStyle}
        >
          <action.icon size={20} />
